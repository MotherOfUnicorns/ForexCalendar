...
def getEconomicCalendar(startlink, endlink):
    ...
    for tr in trs:
        ...
        try:
            for field in fields:
                data = tr.select(
                    "td.calendar__cell.calendar__{}.{}".format(field, field)
                )[0]
                # print(data)
            ...
            dt = datetime.datetime.strptime(
                ",".join([curr_year, curr_date, curr_time]), "%Y,%a%b %d,%I:%M%p"
            )
            print(
                ",".join(
                    [
...
if __name__ == "__main__":
    ...
    """
    setLogger()
    getEconomicCalendar("calendar.php?week=jan7.2017", "calendar.php?week=sep23.2019")
